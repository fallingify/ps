...
public class Main {    
    ...
    private static int bfs(int x, int y) {
        ...
        visited[x][y][0] = true; //0은 벽을 안부심 & 방문한 노드의 방문 여부
        visited[x][y][1] = true; //1은 벽을 부심 &  방문한 노드의 방문 여부
            ...
            for (int i = 0; i < 4; i++) {
                ...
                if (nx >= 0 && nx < n && ny >= 0 && ny < m) {
                    if(board[nx][ny] == 0) { //벽 아닐 때
                        ...
                        if (current.wall == 0 && visited[nx][ny][1] == false) { // 방문한 적이 없을 때 
    ...
    private static class Node {
        ...
        private int y;
        private int count;
        private int wall; //벽뿌심 여부. 0 : 아님, 1: 밪음
